#!/usr/bin/env python3
"""
Complete and accurate conversion of 03.08-scripting.Rmd to PreTeXt XML format
Handles all 456 lines with proper structure and formatting
"""
...
class RmdToPreTeXt:
    ...
    def convert_inline_formatting(self, text, escape=True):
        ...
        # Protect code blocks temporarily AND escape XML in them
        code_parts = []
        def save_code(match):
            code_content = match.group(1)
            # Escape XML in code content
            code_content = self.escape_xml_text(code_content)
        ...
        # Restore code as <c>code</c> (code is already escaped)
    ...
    def process_code_block(self):
        ...
        if fig_cap:
            # Generate figure ID from label, replacing underscores with hyphens
            fig_id = f'fig-{label.replace("_", "-")}' if label else ''
            if fig_id:
    ...
    def parse_code_block_params(self, params_str):
        ...
        for j, part in enumerate(parts):
            ...
            elif j == 0 and 'label' not in params:
    ...
    def convert(self, input_file, output_file):
        ...
        self.output.append('')
        self.output.append('<chapter xml:id="ch8-scripting">')
        self.output.append('  <title>Basic programming</title>')
...
def main():
    input_file = '/home/runner/work/rbook/rbook/bookdown/03.08-scripting.Rmd'
    output_file = '/home/runner/work/rbook/rbook/pretext/source/ch8-scripting.ptx'
